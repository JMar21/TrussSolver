from geom2d import Segment, Rect, Circle, Polygon, Point, Vector
...
def arrow(_segment: Segment, length: float, height:float, attributes=()):
    director = _segment.dir_vec
    v_l = director.antiparallel().with_length(length)
    v_h1 = director.perpVector().with_length(height / 2.0)
    v_h2 = v_h1.antiparallel()
    return group([
        segment(_segment),
        polyline([_segment.end.displacedBy(v_l + v_h1), _segment.end, _segment.end.displacedBy(v_l + v_h2)])
    ],
        attributes
                 )
...
def polygon(poly: Polygon, attributes=()):
    return __polygon_template \
        .replace('{{points}}', __format_points(poly.vertices)) \
        .replace('{{attrs}}', attrs_to_str(attributes))
