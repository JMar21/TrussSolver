...
__STRESS_DISP = 10
__DECIMAL_POS = 4
def __bar_svg(geometry, color, area):
    section_height = sqrt(area)
    return svg.segment(geometry, [attributes.stroke_color(color), attributes.stroke_width(section_height)])
...
def bars_to_svg(bars: [StrBarSolution], settings, config):
    def original_bar_to_svg(_bar: StrBarSolution):
        ...
        return __bar_svg(_bar.original_geometry, color, _bar.area)

    def bar_to_svg(_bar: StrBarSolution):
        return __bar_svg(_bar.final_geometry_disp(settings.disp_scale), bar_color(_bar),\
                         _bar.area)

    def bar_stress_to_svg(_bar: StrBarSolution):
        geometry = _bar.final_geometry_disp(settings.disp_scale)
        normal = geometry.norm_vers
        position = geometry.midpoint.displacedBy(normal, __STRESS_DISP)
        angle = geometry.dir_vers.signed_angle(__I_VERSOR)
        return caption_to_svg(
            f'σ = {round(_bar.stress, __DECIMAL_POS)}',
            position,angle,bar_color(_bar),config
        )
