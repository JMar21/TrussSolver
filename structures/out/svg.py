...
__expected_settings = {
    'scale', 'disp_scale', 'load_scale', 'no_draw_original'
}
def structure_solution_to_svg(result: StructureSolution, settings, _config= None):
    ...
    viewbox = result.boundary_rect(config['sizes']['margin'], settings.disp_scale)
    ...
    return svg.svg_content(size = viewbox.size,
                           primitives=svg_bars + svg_nodes + svg_react + svg_loads,
                           viewbox_rect=viewbox, transform=transform)
def __read_config():
...
def __validate_settings(settings):
    for setting in __expected_settings:
        if setting not in settings:
            raise ValueError(f'"{setting}" not in settings')